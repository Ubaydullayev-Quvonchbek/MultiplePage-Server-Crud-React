...
function About() {
  ...
  const getData = () => {
    axios.get('https://my-server-l1g8.onrender.com/posts').then(res => {
      setData(res.data)
    })
  }
  useEffect(() => {
  ...
  const DeleteData = id => {
    axios.delete(`https://my-server-l1g8.onrender.com/posts/${id}`).then(res => {
      setData(data?.filter(el => el.id !== id))
      alert(res.statusText)
    })
  }
  const editData = (id) => {
    axios.put(`https://my-server-l1g8.onrender.com/posts/${id}`, {
      username: value,
      email: "pro@gmail.com"
    })
    ...
    <>
      <header>
        <div
          ...
          style={{ display: 'block', transform: `${modal}`, transition: ".5s" }}
        >
          <Modal.Dialog>
            <Modal.Header>
